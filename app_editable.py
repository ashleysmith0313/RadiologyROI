        ...
        if col not in df.columns:
            if col in ["description","apc_example"]:
                ...
                if "description" in df.columns:
                    df[col] = df["description"].astype(str).str.lower().str.extract(
                        r"(x[- ]?ray|ct.*no|ct.*with|mri.*no|mri.*with|ultra|pet)", expand=False
                    ).fillna("")
                else:
                    df[col] = ""
...
if page == "Home":
    ...
    with c1:
        ...
        st.write("Optional — you can also type values later.")
        rates_file = st.file_uploader("Rates CSV", type=["csv"], key="rates_csv")
        excel_file = st.file_uploader("Forecast/volumes Excel", type=["xlsx","xls"], key="xlsx")
        pdf_files = st.file_uploader("Productivity PDFs (one or more)", type=["pdf"], accept_multiple_files=True, key="pdfs")
...
if page == "Inputs":
    ...
    with st.expander("Program & finance", expanded=True):
        ...
        pay_comm = colf[3].number_input("Commercial %", value=71, min_value=0, max_value=100, step=1)
        colf2 = st.columns(2)
...
if page == "Results":
    ...
    if "inputs" not in st.session_state or "rates" not in st.session_state:
        ...
        payer_mix = {"commercial": I["pay_comm"]/100.0, "medicaid": I["pay_mcaid"]/100.0, "medicare": I["pay_mcare"]/100.0}
        ...
        # Store for Export page to reuse
        st.session_state["results"] = dict(
            inc_reads=int(round(inc_reads)), net_inc_rev=float(net_inc_rev),
            gross_before_bd=float(gross_before_bd),
            bad_debt_loss=float(gross_before_bd - inc_rev),
            locum_cost=float(I["locum_cost"]), net_gain=float(net_gain)
        )
        ...
        st.session_state["detail_df"] = detail
...
if page == "Export":
    ...
    if "inputs" not in st.session_state or "rates" not in st.session_state or "results" not in st.session_state:
        ...
        # Build waterfall image from RS
        gross_before_bd = RS["gross_before_bd"]
        bad_debt_loss = RS["bad_debt_loss"]
        locum_cost = RS["locum_cost"]
        net_gain = RS["net_gain"]
        steps = [gross_before_bd, -bad_debt_loss, -locum_cost]
        labels = ["Gross incremental revenue", "Bad debt/denials", "Locums cost"]
        ...
        _lab(len(steps), net_gain, 0); ax.set_ylabel("USD"); plt.tight_layout()
        ...
        c.drawString(1*inch, y, f"Bad debt/denials: -${bad_debt_loss:,.0f}"); y -= 0.18*inch
        c.drawString(1*inch, y, f"Locums cost: -${locum_cost:,.0f}"); y -= 0.18*inch
